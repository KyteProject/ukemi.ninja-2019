...
const AuthorMeta = ( { data, settings, canonical } ) => {
	const ghostSettings = settings.allGhostSettings.edges[ 0 ].node,
		author = getAuthorProperties( data ),
		shareImage = author.image || _.get( ghostSettings, 'cover_image', null ),
		title = `${data.name} - ${ghostSettings.title}`,
		description = data.bio || config.siteDescriptionMeta || ghostSettings.description;
