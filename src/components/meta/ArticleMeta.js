...
const ArticleMetaGhost = ({ data, settings, canonical }) => {
  ...
  const shareImage = ghostPost.feature_image
    ...
    : _.get(ghostSettings, "cover_image", null);
  const publisherLogo =	ghostSettings.logo || config.siteIcon	? url.resolve( config.siteUrl, ghostSettings.logo || config.siteIcon )	: null; // prettier-ignore
