...
const ArticleMetaGhost = ( { data, settings, canonical } ) => {
	const ghostPost = data,
		...
		author = getAuthorProperties( ghostPost.primary_author ),
		publicTags = _.map( tagsHelper( ghostPost, { visibility: 'public', fn: ( tag ) => tag } ), 'name' ),
		primaryTag = publicTags[ 0 ] || '',
		shareImage = ghostPost.feature_image ? ghostPost.feature_image : _.get( ghostSettings, 'cover_image', null ),
		publisherLogo =	ghostSettings.logo || config.siteIcon	? url.resolve( config.siteUrl, ghostSettings.logo || config.siteIcon )	: null; // prettier-ignore
