...
const createPages = async ({ graphql, actions: { createPage } }) => {
  ...
  tags.forEach(({ node }) => {
    ...
    const url = `blog/tag/${node.slug}/`;

    Array.from({ length: numberOfPages }).forEach((_, i) => {
      const currentPage = i + 1;
      const prevPageNumber = currentPage <= 1 ? null : currentPage - 1;
      const nextPageNumber = currentPage + 1 > numberOfPages ? null : currentPage + 1;
      const nextPagePath = nextPageNumber ? `${url}page/${nextPageNumber}/` : null;
      const previousPagePath = prevPageNumber	? prevPageNumber === 1 ? url : `${url}page/${prevPageNumber}/` : null; // prettier-ignore
  ...
  authors.forEach(({ node }) => {
    ...
    const url = `blog/author/${node.slug}/`;
